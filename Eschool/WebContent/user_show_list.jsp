...
<body>
	<div class="pd-20">
				...
				<% 
					if(request.getAttribute("List")!=null){
						...
						for(Show show:list){
							...
							out.print("<td><a href='ShowsServlet?type=delete&show_id="+show.getShowId()+"' onclick=\"if(confirm('确定删除吗?')==false)return false;\">删除</a></td>");
							out.print("</tr>");
						}
					}
				%>
		...
		<center>
			...
			%><a
				href="ShowsServlet?type=pages&currentpageno=<%=currentpageno + 1%>">下一页</a>
			<a href="ShowsServlet?type=pages&currentpageno=<%=pageCount%>">尾页</a>
			<%
				} else if (currentpageno > 1 && currentpageno < pageCount) {
			%>
			<a href="ShowsServlet?type=pages&currentpageno=1">首页</a> <a
				href="ShowsServlet?type=pages&currentpageno=<%=currentpageno - 1%>">上一页</a>
			<a
				href="ShowsServlet?type=pages&currentpageno=<%=currentpageno + 1%>">下一页</a>
			<a href="ShowsServlet?type=pages&currentpageno=<%=pageCount%>">尾页</a>
			<%
